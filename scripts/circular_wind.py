...
# %%
def U_mask_fun(i, j):
    x, y = i / grid.n[0], j / grid.n[1]
    m = 0 if (x in (0,) or x**2 + (y - 0.5) ** 2 >= 0.5**2) else 1
    return m
...
beta = GridFunction.from_function(beta_fun, grid)


def B_form(u, v):
    return epsilon * dot(grad(u, "+"), grad(v, "+")) + dot(beta, grad(u, "+")) * v
...
ys = np.linspace(-1.0, 1.0, grid.n[1] + 1)
gamma1 = 0.5 * (np.tanh((0.65 - np.abs(ys)) / epsilon) + 1)
gamma2 = 0.5 * (np.tanh((np.abs(ys) - 0.35) / epsilon) + 1)

shift_arr[0, :] = np.where(np.abs(ys) > 0.5, gamma1, gamma2)
...
lower_bound = 1 / 4 * np.sqrt(loss)
upper_bound = 1 / epsilon * np.sqrt(loss)
